(function() {
	...
	var e = {
		...
		mimeTypes : g,
		extend : function(l) {
			e.each(arguments, function(m, n) {
				if (n > 0) {
					e.each(m, function(p, o) {
						l[o] = p
					})
				}
			});
		...
		cleanName : function(l) {
			var m, n;
			n = [ /[\300-\306]/g, "A", /[\340-\346]/g, "a", /\307/g, "C",
					/\347/g, "c", /[\310-\313]/g, "E", /[\350-\353]/g, "e",
					/[\314-\317]/g, "I", /[\354-\357]/g, "i", /\321/g, "N",
					/\361/g, "n", /[\322-\330]/g, "O", /[\362-\370]/g, "o",
					/[\331-\334]/g, "U", /[\371-\374]/g, "u" ];
			for (m = 0; m < n.length; m += 2) {
				l = l.replace(n[m], n[m + 1])
			}
			l = l.replace(/\s+/g, "_");
			l = l.replace(/[^a-z0-9_\-\.]+/gi, "");
			return l
		},
		addRuntime : function(l, m) {
			m.name = l;
			i[l] = m;
			i.push(m);
			return m
		...
		guid : function() {
			var l = new Date().getTime().toString(32), m;
			for (m = 0; m < 5; m++) {
				l += Math.floor(Math.random() * 65535).toString(32)
			}
			return (e.guidPrefix || "p") + l + (d++).toString(32)
		},
		buildUrl : function(m, l) {
			var n = "";
			e.each(l, function(p, o) {
				n += (n ? "&" : "") + encodeURIComponent(o) + "="
		...
		formatSize : function(l) {
			if (l === b) {
				return e.translate("N/A")
			}
			if (l > 1048576) {
				return Math.round(l / 1048576, 1) + " MB"
			}
			if (l > 1024) {
				return Math.round(l / 1024, 1) + " KB"
			}
			return l + " b"
		},
		getPos : function(m, q) {
			var r = 0, p = 0, t, s = document, n, o;
			m = m;
			q = q || s.body;
			function l(z) {
				var v, w, u = 0, A = 0;
				if (z) {
					...
					v = s.compatMode === "CSS1Compat" ? s.documentElement
							: s.body;
					u = w.left + v.scrollLeft;
					A = w.top + v.scrollTop
				}
				return {
					x : u,
		...
		getSize : function(l) {
			return {
				w : l.clientWidth || l.offsetWidth,
				h : l.clientHeight || l.offsetHeight
			}
		},
		parseSize : function(l) {
			var m;
			if (typeof (l) == "string") {
				l = /^([0-9]+)([mgk]+)$/.exec(l.toLowerCase().replace(
						/[^0-9mkg]/g, ""));
				m = l[2];
				l = +l[1];
				if (m == "g") {
		...
		xmlEncode : function(l) {
			return l ? ("" + l).replace(j, function(m) {
				return a[m] ? "&" + a[m] + ";" : m
			}) : l
		},
		toArray : function(n) {
	...
	e.Uploader = function(o) {
		var m = {}, r, q = [], s, n;
		r = new e.QueueProgress();
		o = e.extend({
			chunk_size : 0,
			multipart : true,
		...
		function p() {
			var t;
			if (this.state == e.STARTED && s < q.length) {
				t = q[s++];
				if (t.status == e.QUEUED) {
					t.status = e.UPLOADING;
					this.trigger("BeforeUpload", t);
					this.trigger("UploadFile", t)
				} else {
					p.call(this)
				}
			} else {
		...
		function l() {
			...
			if (r.size === b) {
				r.percent = q.length > 0 ? Math.ceil(r.uploaded / q.length
						* 100) : 0
			} else {
				r.bytesPerSec = Math.ceil(r.loaded
						/ ((+new Date() - n || 1) / 1000));
		...
		e.extend(this, {
			...
			id : e.guid(),
			init : function() {
				var y = this, z, v, u, x = 0, w;
				if (typeof (o.preinit) == "function") {
					o.preinit(y)
				} else {
					e.each(o.preinit, function(B, A) {
						y.bind(A, B)
					})
				}
				o.page_url = o.page_url
						|| document.location.pathname
								.replace(/\/[^\/]+$/g, "/");
				if (!/^(\w+:\/\/|\/)/.test(o.url)) {
					o.url = o.page_url + o.url
				}
				o.chunk_size = e.parseSize(o.chunk_size);
				o.max_file_size = e.parseSize(o.max_file_size);
				y.bind("FilesAdded", function(A, D) {
					var C, B, F = 0, G, E = o.filters;
					if (E && E.length) {
						G = [];
						e.each(E, function(H) {
							e.each(H.extensions.split(/,/), function(I) {
								G.push("\\."
					...
					for (C = 0; C < D.length; C++) {
						...
						if (B.size !== b && B.size > o.max_file_size) {
							A.trigger("Error", {
				...
				if (o.unique_names) {
					y.bind("UploadFile", function(A, B) {
						var D = B.name.match(/\.([^.]+)$/), C = "tmp";
						if (D) {
							C = D[1]
						}
						B.target_name = B.id + "." + C
					})
				}
				y.bind("UploadProgress", function(A, B) {
					B.percent = B.size > 0 ? Math.ceil(B.loaded / B.size * 100)
							: 100;
					l()
				});
				y.bind("StateChanged", function(A) {
					if (A.state == e.STARTED) {
						n = (+new Date())
					}
				});
				y.bind("QueueChanged", l);
				y.bind("Error", function(A, B) {
					if (B.file) {
						B.file.status = e.FAILED;
						l();
						c(function() {
							p.call(y)
						})
				...
				y.bind("FileUploaded", function(A, B) {
					B.status = e.DONE;
					B.loaded = B.size;
					A.trigger("UploadProgress", B);
					c(function() {
						p.call(y)
					})
				});
				...
				function t() {
					...
					if (D) {
						C = D.getFeatures();
						A = y.settings.required_features;
						if (A) {
							A = A.split(",");
			...
			getFile : function(u) {
				var t;
				for (t = q.length - 1; t >= 0; t--) {
					if (q[t].id === u) {
						return q[t]
					}
				}
			},
			removeFile : function(u) {
				var t;
				for (t = q.length - 1; t >= 0; t--) {
					if (q[t].id === u.id) {
			...
			splice : function(v, t) {
				var u;
				u = q.splice(v === b ? 0 : v, t === b ? q.length : t);
				this.trigger("FilesRemoved", u);
				this.trigger("QueueChanged");
				return u
			},
			trigger : function(u) {
				var w = m[u.toLowerCase()], v, t;
				if (w) {
					t = Array.prototype.slice.call(arguments);
					t[0] = this;
					for (v = 0; v < w.length; v++) {
						if (w[v].func.apply(w[v].scope, t) === false) {
							return false
						}
					}
				}
	...
	e.QueueProgress = function() {
		...
		l.bytesPerSec = 0;
		l.reset = function() {
			l.size = l.loaded = l.uploaded = l.failed = l.queued = l.percent = l.bytesPerSec = 0
		}
	};
	e.runtimes = {};
	window.plupload = e
...
(function(b) {
	var c = {};
	function a(j, f, m, l, d) {
		var n, h, g, i;
		h = google.gears.factory.create("beta.canvas");
		try {
			h.decode(j);
			i = Math.min(f / h.width, m / h.height);
			if (i < 1) {
				h.resize(Math.round(h.width * i), Math.round(h.height * i));
				return h.encode(d, {
					quality : l / 100
				})
			}
							...
							g
									.bind(
											...
											function(o, l) {
												...
												function j() {
													var v, x, s = o.settings.multipart, r = 0, w = {
														name : l.target_name
																|| l.name
													}, t = o.settings.url;
													function u(z) {
														var y, E = "----pluploadboundary"
																+ b.guid(), B = "--", D = "\r\n", A, C;
														if (s) {
															v
																	.setRequestHeader(
					...
					"silverlight",
					{
						...
						init : function(l, m) {
							...
							c.extend(k.style, {
								...
								height : "100px",
								overflow : "hidden",
								opacity : l.settings.shim_bgcolor
...
(function(d) {
	...
	function b(g, h) {
		...
		if ("FileReader" in window) {
			...
			return h(g.getAsDataURL())
		}
	}
	function a(l, n, k, f, m) {
		var h, g, j, i;
		b(l, function(o) {
			...
			j.onload = function() {
				var s, p, q, r, t;
				i = Math.min(n / j.width, k / j.height);
				if (i < 1) {
					s = Math.round(j.width * i);
					p = Math.round(j.height * i);
					h.width = s;
					h.height = p;
					g.drawImage(j, 0, 0, s, p);
					t = new c();
					t.init(atob(o.substring(o.indexOf("base64,") + 7)));
					r = t.APP1({
						width : s,
						height : p
					});
					o = h.toDataURL(f);
					o = o.substring(o.indexOf("base64,") + 7);
					o = atob(o);
					if (r) {
						t.init(o);
						t.setAPP1(r);
	...
	d.runtimes.Html5 = d
			.addRuntime(
					...
					{
						getFeatures : function() {
							var k, g, j, h, f, i = window;
							g = j = h = f = false;
							if (i.XMLHttpRequest) {
								k = new XMLHttpRequest();
								j = !!k.upload;
								g = !!(k.sendAsBinary || k.upload)
							}
							if (g) {
								h = !!(File
										&& (File.prototype.getAsDataURL || i.FileReader) && k.sendAsBinary);
								f = !!(File && File.prototype.slice)
							}
							e = navigator.userAgent.indexOf("Safari") > 0;
							return {
								html5 : g,
								dragdrop : i.mozInnerScreenX !== undefined || f
										|| e,
								jpgresize : h,
								pngresize : h,
								multipart : h || !!i.FileReader || !!i.FormData,
								progress : j,
								chunking : f || h
							}
						},
						init : function(i, j) {
							...
							function h(n) {
								...
								for (k = 0; k < n.length; k++) {
									...
									f[o] = l;
									m
											.push(new d.File(o, l.fileName,
													l.fileSize))
								}
								if (m.length) {
									i.trigger("FilesAdded", m)
								}
							}
							g = this.getFeatures();
							if (!g.html5) {
							...
							i
									.bind(
											"Init",
											function(o) {
												var s, q = [], n, r, l = o.settings.filters, m, p, k = document.body;
												s = document
														.createElement("div");
												s.id = o.id
														+ "_html5_container";
												...
												d
														.extend(
																...
																{
																	position : "absolute",
																	background : i.settings.shim_bgcolor
																			|| "transparent",
																	width : "100px",
																	height : "100px",
																	overflow : "hidden",
																	zIndex : 99999,
																	opacity : i.settings.shim_bgcolor ? ""
																			: 0
																});
												s.className = "plupload html5";
												if (i.settings.container) {
													k = document
															.getElementById(i.settings.container);
													k.style.position = "relative"
												}
												k.appendChild(s);
												s.innerHTML = '<input id="'
														+ i.id
														+ '_html5" style="width:100%;" type="file" accept="'
														+ q.join(",")
														+ '" '
														+ (i.settings.multi_selection ? 'multiple="multiple"'
																: "") + " />";
							...
							i
									.bind(
											...
											function() {
												var k = document
														.getElementById(i.settings.drop_element);
												if (k) {
													if (e) {
														d
																.addEvent(
							...
							i
									.bind(
											...
											function(k) {
												var l, m, n;
												l = document
														.getElementById(i.settings.browse_button);
												m = d
														.getPos(
																l,
																document
																		.getElementById(k.settings.container));
												n = d.getSize(l);
												d
														.extend(
																document
							...
							i
									.bind(
											"UploadFile",
											function(k, m) {
												var n = k.settings, p, l;
												function o(q) {
													var t = 0, s = 0;
													function r() {
														var A = q, H, I, D, E, F = 0, w = "----pluploadboundary"
																+ d.guid(), z, B, x, y = "--", G = "\r\n", C = "", v, u = k.settings.url;
														if (m.status == d.DONE
																|| m.status == d.FAILED
																|| k.state == d.STOPPED) {
															return
														...
														if (k.settings.multipart
																	...
																	.each(
																			...
																			function(
																				...
																				C += y
																						...
																						+ G
																						+ G;
																				C += unescape(encodeURIComponent(K))
																						+ G
																			});
															v = d.mimeTypes[m.name
																	.replace(
																			/^.+\.([^.]+)/,
																			"$1")]
																	|| "application/octet-stream";
															C += y
																	...
																	+ k.settings.file_data_name
																	+ '"; filename="'
																	+ unescape(encodeURIComponent(m.name))
																	+ '"'
																	+ G
																	+ "Content-Type: "
																	+ v + G + G
												...
												if (g.jpgresize) {
													if (l
														...
														a(
																...
																l.height,
																/\.png$/i
																		.test(m.name) ? "image/png"
																		: "image/jpeg",
																function(q) {
																	if (q.success) {
																		m.size = q.data.length;
																		o(q.data)
																	} else {
																		o(p
																				.getAsBinary())
	...
	c = function() {
		var h, i, x, r, s, m, q, t, C, o, z, v, j, B, f, A, y, k, g;
		function n() {
			var G = false, E;
			function H(J, L) {
				var I = G ? 0 : -8 * (L - 1), M = 0, K;
				for (K = 0; K < L; K++) {
					M |= (E.charCodeAt(J + K) << Math.abs(I + K * 8))
				}
				return M
			}
			function D(I, K, J) {
				E = E.substr(0, I) + K
						+ E.substr((J === true ? K.length : 0) + I)
			}
			function F(J, K, M) {
				var N = "", I = G ? 0 : -8 * (M - 1), L;
				for (L = 0; L < M; L++) {
					N += String.fromCharCode((K >> Math.abs(I + L * 8)) & 255)
				}
				D(J, N, true)
			}
			return {
				...
				SEGMENT : function(I, K, J) {
					if (!arguments.length) {
						return E
					}
					if (typeof K == "number") {
						return E.substr(parseInt(I, 10), K)
					}
					D(I, K, J)
				},
				BYTE : function(I) {
				...
				SLONG : function(I) {
					var J = H(I, 4);
					return (J > 2147483647 ? J - 4294967296 : J)
				},
				STRING : function(I, J) {
					var K = "";
					for (J += I; I < J; I++) {
						K += String.fromCharCode(H(I, 1))
					}
					return K
				}
			}
		...
		function p(D, K) {
			var F = C.SHORT(D), H, N, P, J, I, E, G, L, M = [], O = {};
			for (H = 0; H < F; H++) {
				G = E = D + 12 * H + 2;
				P = K[C.SHORT(G)];
				if (P === f) {
				...
				case 5:
					G = C.LONG(G) + B;
					for (N = 0; N < I; N++) {
						M[N] = C.LONG(G + N * 4) / C.LONG(G + N * 4 + 4)
					}
					break;
				case 9:
					G = C.LONG(G) + B;
				...
				case 10:
					G = C.LONG(G) + B;
					for (N = 0; N < I; N++) {
						M[N] = C.SLONG(G + N * 4) / C.SLONG(G + N * 4 + 4)
					}
					break;
				default:
					continue
				...
				L = (I == 1 ? M[0] : M);
				if (g.hasOwnProperty(P) && typeof L != "object") {
					O[P] = g[P][L]
				} else {
					O[P] = L
				}
		...
		function w() {
			var D = t + 4;
			B += t;
			if (C.STRING(D, 4).toUpperCase() !== "EXIF"
					|| C.SHORT(D += 4) !== 0) {
				return
			}
			C.II(C.SHORT(D += 2) == 18761);
			if (C.SHORT(D += 2) !== 42) {
				return
			}
			j = B + C.LONG(D += 2);
			h = p(j, A);
			z = ("ExifIFDPointer" in h ? B + h.ExifIFDPointer : f);
			v = ("GPSInfoIFDPointer" in h ? B + h.GPSInfoIFDPointer : f);
			return true
		}
		function l(D, I, H) {
			var G = D.SHORT(H), F, E;
		...
		return {
			...
			EXIF : function() {
				i = p(z, y);
				i.ExifVersion = String.fromCharCode(i.ExifVersion[0],
						i.ExifVersion[1], i.ExifVersion[2], i.ExifVersion[3]);
				return i
			},
			GPS : function() {
				x = p(v, k);
						...
						init : function(c, d) {
							c
									.bind(
											"Init",
											function(m) {
												var f = document.body, k, e = "javascript", g, u, n, o = /MSIE/
														.test(navigator.userAgent), q = [], j = m.settings.filters, l, h, p, t;
												...
												function s() {
													...
													i = m.settings.shim_bgcolor;
													if (i) {
														w.style.background = i
													}
													if (o) {
														a
																.extend(
												...
												function r() {
													...
													k.setAttribute("src", e
															+ ':""');
													k.setAttribute("id", m.id
															+ "_iframe");
													k.setAttribute("name", m.id
															+ "_iframe");
													...
													a
															.addEvent(
																	...
																	function(x) {
																		...
																		try {
																			w = y.contentWindow.document
																					|| y.contentDocument
																					|| window.frames[y.id].document
																		} catch (v) {
																			m
																					.trigger(
																							"Error",
																		...
																		i = w.documentElement.innerText
																				|| w.documentElement.textContent;
																		if (i) {
																			g.status = a.DONE;
																			g.loaded = 1025;
																			g.percent = 100;
												...
												m
														.bind(
																"StateChanged",
																function(i) {
																	if (i.state == a.STARTED) {
																		r()
																	}
																	if (i.state == a.STOPPED) {
																		window
																				.setTimeout(
																						function() {
																							k.parentNode
												...
												m
														.bind(
																...
																function(i) {
																	var v, w, x;
																	v = b(i.settings.browse_button);
																	w = a
																			.getPos(
																					v,
																					b(i.settings.container));
																	x = a
																			.getSize(v);
																	a
																			.extend(
